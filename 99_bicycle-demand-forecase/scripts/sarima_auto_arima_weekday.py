...
Usage:
    # terminal
    python scripts/sarima_auto_arima_weekday.py
...
# ── Build weekday series ──────────────────────────────────────────────────────
series   = make_series(df_raw, str(TARGET_RENT_ID))
series_weekday = series[series.index.dayofweek < 5].dropna()
print(f"Weekday train : {series_weekday.index[0].date()} ~ {series_weekday.index[-1].date()}  "
      f"({len(series_weekday):,} hours)")
...
model = model.fit(series_weekday.values)
...
print(f"{'='*50}")
print(f"\n# Paste into 01_sarima_weekday.ipynb Section 6:")
