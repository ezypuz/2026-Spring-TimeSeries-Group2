...
\begin{document}
	...
	\subsection{연구 목적}
	본 연구는 시공간적 데이터에 기반한 수요 예측 모델을 구축하여, 자전거 재배치의 우선순위와 최적 경로를 도출하고 운영 효율화를 위한 데이터 기반의 정책적 근거를 마련하는 데 목적이 있다. \\ 특히 본 프로젝트는 시계열 예측 문제를 명확한 연구 목표로 설정하고, SARIMA 및 ARIMAX 기반 모델을 활용하여 단기 수요 예측 성능을 정량적으로 평가하는 것을 목표로 한다. 이를 위해 예측 정확도 지표로 MAPE, RMSE 등을 활용하며, 최종적으로는 모델이 일정 수준 이하의 예측 오차를 달성하는 것을 목표로 한다. 최종적으로는 구축된 시계열 모델의 성능을 머신러닝 기반 예측 모델(예: Random Forest, XGBoost 등)과 비교하여, 전통적 시계열 모델과 머신러닝 접근법 간의 성능 차이를 정량적으로 분석하고자 한다. 이를 통해 각 접근법의 장단점을 명확히 하고, 실제 운영 환경에서 적용 가능한 최적의 수요 예측 방법론을 제시하는 것을 궁극적인 목표로 한다.
